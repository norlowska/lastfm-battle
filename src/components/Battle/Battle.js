...
class Battle extends Component {
  ...
  calculateWins(user1, user2) {
    ...
    var u1scores = [
      Number(user1.playcount),
      Number(user1.scrobblesThisYear),
      Number(user1.scrobblesThisMonth),
      Number(user1.scrobblesThisWeek)
    ];
    var u2scores = [
      Number(user2.playcount),
      Number(user2.scrobblesThisYear),
      Number(user2.scrobblesThisMonth),
      Number(user2.scrobblesThisWeek)
    ];
    ...
    if (error) {
      mainContent = (
        <div className="center-container">
          <p className="error-message">An error ocurred: {error.message}.</p>
        </div>
      );
    } else if (isPending) {
      mainContent = (
